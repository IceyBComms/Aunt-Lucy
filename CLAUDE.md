...
> ⚠️ **BEFORE YOU DO ANYTHING ELSE IN A SESSION: run `git status` on `build/BUGS_AND_FIXES.md` and commit any uncommitted changes.** **Cowork edits that file directly on disk between your sessions, so the working tree is frequently AHEAD of the last commit and is the authoritative version.** **On 23 August 2026 an aborted `git checkout` had already removed `build/AUDIT_FINDINGS.md` from the working tree and came within one forced checkout of destroying Kate's edits to the bug record. Never force a checkout over that file. Diff first, preserve the newer version, then proceed. You are not the only writer.** **⚠️ AND COMMITTING IS NOT ENOUGH — THE BUG RECORD HAS TO REACH `main`.** **A commit on a feature branch is only safe if that branch is one you are actually going to merge. If the branch is already spent — its PR merged, or the work on it closed — the edits are STRANDED there, and deleting the branch destroys them. This happened on 23 August 2026: the session correctly committed Kate's edits first thing, then committed her end-of-night edits to a branch whose PR had already merged an hour earlier, leaving `#033 → 🟢` and five new bugs one branch-deletion from gone. Doing the first rule right does not save you from this one.** **So: after committing, check `git log --oneline origin/main..HEAD` and ask whether that commit has a route to `main`. If it doesn't, put the bug-record edits on a FRESH branch off `origin/main` and open a PR for them alone — small, readable, and merged on their own rather than cherry-picked later.**
